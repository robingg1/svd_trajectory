...
from utils.dataset import WebVid10M, Trajectory_Data
...
def load_images_trajs(folders):
    all_images = []
    all_trajs = []
    for folder in os.listdir(folders):
        images, trajs = load_images_trajs_from_folder(os.path.join(folders, folder))
        all_images.append(images)
        all_trajs.append(trajs)
    return all_images, all_trajs

def load_images_trajs_from_folder(folder):
    images = []
    trajs = []
    valid_extensions = {".jpg", ".jpeg", ".png", ".bmp", ".gif", ".tiff"}  # Add or remove extensions as needed
    ...
    # print(image_files)
    traj_files = sorted(os.listdir(os.path.join(folder, "traj_vid", "0000_0016")))
    # print(traj_files)
    ...
    # Load images in sorted order
    for filename in image_files[:16]:
        ext = os.path.splitext(filename)[1].lower()
        if ext in valid_extensions:
    ...
    for filename in traj_files:
        ext = os.path.splitext(filename)[1].lower()
        if ext in valid_extensions:
            img = Image.open(os.path.join(folder, "traj_vid", "0000_0016", filename)).convert('RGB')
            trajs.append(img)
...
def make_train_traj_dataset(args):
    # Get the datasets: you can either provide your own training and evaluation files (see below)
    # or specify a Dataset from the hub (the dataset will be downloaded automatically from the datasets Hub).

    # In distributed training, the load_dataset function guarantees that only one local process can concurrently
    # download the dataset.
    dataset = Trajectory_Data(path=args.video_folder)
    return dataset
...
def main():
    ...
    train_dataset = make_train_traj_dataset(args)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if (
                        ...
                        validation_images, validation_control_images = load_images_trajs(args.validation_image_folder)
                        ...
                        with torch.autocast(
                            ...
                            for val_img_idx in range(args.num_validation_images):
                                ...
                                video_frames = pipeline(
                                    ...
                                    validation_control_images[val_img_idx][:16],
                                    height=args.height,
                                    width=args.width,
                                    num_frames=16,
                                    decode_chunk_size=8,
                                ...
                                save_combined_frames(video_frames, validation_images[val_img_idx][:16], validation_control_images[val_img_idx][:16],val_save_dir)
